...
namespace {
  ...
  void mesh_to_surfel (vector<Eigen::Vector3f> const& vertices, vector<array<unsigned int, 3>> const& faces, vector<Surfel>& surfels);
  //{{{
  void load_dragon() {
  ...
  void steiner_circumellipse (float const* v0_ptr, float const* v1_ptr,
    ...
    Vector3f d0, d1, d2;
      ...
      Matrix3f A;
      for (unsigned int j(0); j < 3; ++j)
        A.row(j) = Vector3f (p[j].x() * p[j].x(), 2.0f * p[j].x() * p[j].y(), p[j].y() * p[j].y());
  ...
  void face_to_surfel (vector<Eigen::Vector3f> const& vertices,
    ...
    Vector3f v[3] = { vertices[face[0]], vertices[face[1]], vertices[face[2]] };
